...
parser.add_argument('-s', '--sliding', action = 'store_true', \
                    help = 'Project imaging by sliding the window.')
...
logger.info("Converting the image to float to avoid overflow.")
...
logger.info("Window width: {0}. Sliding window: {1}.".format(window, sliding))
image_list = []
if sliding:
    for t_index in progressbar(range(input_stack.t_count)):
        channel_list = []
        for c_index in range(input_stack.c_count):
            image = proj_func(image_array[t_index:(t_index + window), c_index], axis = 0)
            channel_list.append(image)
        image_list.append(channel_list)
else:
    for t_index in progressbar(range(0, input_stack.t_count, window)):
        channel_list = []
        for c_index in range(input_stack.c_count):
            image = proj_func(image_array[t_index:(t_index + window), c_index], axis = 0)
            channel_list.append(image)
